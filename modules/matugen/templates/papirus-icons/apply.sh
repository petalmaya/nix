...

{
  ...
  closest=$(
    ...
    function h2d(hex,   i, val, c) {
      val = 0; hex = tolower(hex)
      for (i = 1; i <= length(hex); i++) {
        c = index("0123456789abcdef", substr(hex, i, 1)) - 1
        if (c >= 0) val = val * 16 + c
      }
      return val
    }
    BEGIN {
      ...
      for (i = 1; i <= n; i++) {
        split(arr[i], p, ":")
        cr = h2d(substr(p[2],1,2))
        cg = h2d(substr(p[2],3,2))
        cb = h2d(substr(p[2],5,2))
  ...
  if [[ ! -d $HOME/.local/share/icons/Papirus ]]; then
    ...
    # NixOS does not use /usr/share/icons; check Nix store/system paths first
    if [[ -d "/run/current-system/sw/share/icons/Papirus" ]]; then
      cp -r "/run/current-system/sw/share/icons/Papirus" "$HOME/.local/share/icons/"
    elif [[ -d "$HOME/.nix-profile/share/icons/Papirus" ]]; then
      cp -r "$HOME/.nix-profile/share/icons/Papirus" "$HOME/.local/share/icons/"
    elif [[ -d "/usr/share/icons/Papirus" ]]; then
      cp -r "/usr/share/icons/Papirus" "$HOME/.local/share/icons/"
    else
      # Fallback: search via XDG_DATA_DIRS
      found=""
      IFS=':' read -ra dirs <<< "${XDG_DATA_DIRS:-/run/current-system/sw/share:/usr/share}"
      for d in "${dirs[@]}"; do
        if [[ -d "$d/icons/Papirus" ]]; then
          cp -r "$d/icons/Papirus" "$HOME/.local/share/icons/"
          found=1
          break
        fi
      done
