# builds nagarebar (+ greeter) from the vendored ./quickshell config.
...
pkgs.symlinkJoin {
  ...
  postBuild = ''
    makeWrapper ${pkgs.lib.getExe qs} $out/bin/nagarebar \
      --set FONTCONFIG_FILE "${fontconfig}" \
      --set QML2_IMPORT_PATH "${qmlPath}" \
      --prefix PATH : "${runtimePath}"

    makeWrapper ${pkgs.lib.getExe qs} $out/bin/nagarebar-greeter \
      --set FONTCONFIG_FILE "${fontconfig}" \
      --set QML2_IMPORT_PATH "${qmlPath}" \
      --prefix PATH : "${runtimePath}" \
  ...
  meta = {
    description = "nagare-shell - Quickshell config for niri/mangowc (includes a greetd greeter)";
    mainProgram = "nagarebar";
