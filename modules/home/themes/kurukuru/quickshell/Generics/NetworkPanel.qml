      ...
      Repeater {
        ...
        Rectangle {
          ...
          Gen.MouseArea {
            ...
            onClicked: {
              // unpaired devices only showed up here because a scan found
              // them - pair first (bluez won't let you connect() straight
              // to a stranger device). already-paired devices behave as
              // before: click toggles connect/disconnect.
              if (btEntry.modelData.pairing) {
                btEntry.modelData.cancelPair();
              } else if (!btEntry.modelData.paired) {
                btEntry.modelData.pair();
              } else if (btEntry.modelData.connected) {
