#!/usr/bin/env bash
# Mock greeter preview in the current session (hot reload); TEAKETTLER_GREETER_MOCK=1 fakes greetd, see Data/Greeter.qml.
set -euo pipefail
...
TEAKETTLER_GREETER_MOCK=1 exec quickshell -p "$REPO_ROOT/greeter.qml"
