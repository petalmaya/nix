...
// Same shape as Layers/NetPanel.qml but separate so the two open independently without sharing state.
WlrLayershell {
  id: root
  ...
  function close() {
    ...
    // Collapse the embedded network section on close so its wifi/bluetooth polling stops.
