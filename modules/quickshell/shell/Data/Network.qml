...
// WiFi via `nmcli`; requires NetworkManager, no-op on iwd/systemd-networkd.
Singleton {
  id: root
  ...
  // Splits nmcli terse manually; lookbehind regex throws on some QJSEngine versions.
  ...
  // Polls only while the panel is open (see Globals.anyNetworkPanelOpen).
  Timer {
    interval: 15000
    repeat: true
